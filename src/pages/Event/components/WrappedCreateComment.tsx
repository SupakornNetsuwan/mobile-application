  ...
  return (
    <StyledView>
      <StyledView className="flex-row space-x-1">
        {owner && owner.data.attributes.picture.data != null ? (
          <StyledImage
            source={{
              uri: `${process.env.EXPO_PUBLIC_BACKEND_URL}${owner.data.attributes.picture.data.attributes.url}`,
            }}
          ...
          <StyledView
            className="rounded-full w-12 h-12 bg-purple-primary"
            style={{ borderColor: process.env.EXPO_PUBLIC_PRIMARY_COLOR, borderWidth: 2 }}
          >
        ...
        <StyledView>
          <StyledText className="text-sm font-noto-semibold m-1">
            {owner ? owner.data.attributes.username : "ไม่ผบผู้ใช้"}
          </StyledText>
          <StyledText className="text-xs text-gray-500 pl-1">
...
const PostComponent: React.FC<
  PostType & {
    setOpeningPostModal?: (newType: boolean) => void;
    setPostId?: (newType: number) => void;
    setOwnerId?: (newType: number) => void;
  }
> = ({ attributes, id, eventId, setOpeningPostModal, setOwnerId, setPostId }) => {
        ...
        <StyledView className="flex flex-row justify-between">
          <StyledView className="flex flex-row space-x-1 pt-2 pb-4 px-2">
            {postDetails.owner.data.attributes.picture.data != null ? (
              ...
              <StyledView
                className="rounded-full w-14 h-14 bg-purple-primary"
                style={{ borderColor: process.env.EXPO_PUBLIC_PRIMARY_COLOR, borderWidth: 2 }}
              >
                <MaterialCommunityIcons
                  name="account"
      ...
      <StyledView className="px-2 py-3 space-y-1">
        ...
        <StyledView>
          ...
          <ScrollView nestedScrollEnabled={true} decelerationRate={0.1}>
            {postDetails.comments.data &&
              postDetails.comments.data.length > 0 &&
              Array.from({ length: postDetails.comments.data.length }, (_, index) => (
                <StyledView key={index} className="my-2">
                  <Comment
                    contents={
                      postDetails.comments.data[index].attributes.content &&
                      postDetails.comments.data[index].attributes.content
                    }
                    owner={
                      postDetails.comments.data[index].attributes.owner &&
                      postDetails.comments.data[index].attributes.owner
                    }
                  />
                </StyledView>
        ...
        <StyledView>
          <StyledView className="flex-row justify-between items-center">
            ...
            >
              <MaterialCommunityIcons name="send" size={24} color="rgb(107,114,128)" />
            </StyledTouchableOpacity>
          </StyledView>
        </StyledView>
        {/* Delete Overlay */}
...
const WrappedCreateComment: React.FC<
  PostType & {
    setOpeningPostModal?: (newType: boolean) => void;
    setOwnerId?: (newType: number) => void;
    setPostId?: (newType: number) => void;
...
> = ({ attributes, id, eventId, setOpeningPostModal, setOwnerId, setPostId }) => {
  return (
    <>
      <CommentFormProvider>
        <PostComponent
          setOpeningPostModal={setOpeningPostModal}
